...
if(isset($_GET['id'])){
    ...
    $checkPostQuery = "SELECT * FROM posts WHERE pid = '$postId' AND userId = '$userId'";
    $result = mysqli_query($conn, $checkPostQuery);
    
    if(mysqli_num_rows($result) > 0){
        // Proceed with the deletion - FIXED QUERY
        $deleteQuery = "DELETE FROM posts WHERE pid = '$postId' AND userId = '$userId'";
